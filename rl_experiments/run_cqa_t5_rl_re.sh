...
do
  ...
  --gpu=0 \
  --seed=${seed} \
  --model_name="cqa_t5_rl_re_seed${seed}" \
  ...
  --max_seq_len=110 \
  --write_prediction \
  --train_output_file="cqa_re_train_rl_seed${seed}.csv" \
  --eval_output_file="cqa_re_eval_rl_seed${seed}.csv" \
  --test_output_file="cqa_re_test_rl_seed${seed}.csv" 

  # train task model using generated explanations and predictions
  python t5_rl.py \
  --gpu=0 \
  --seed=${seed} \
  --model_name="cqa_t5_re_rl_sim_seed${seed}" \
  --train_data_file="cqa_re_train_rl_seed${seed}.csv" \
  --eval_data_file="cqa_re_eval_rl_seed${seed}.csv" \
  --test_data_file="cqa_re_test_rl_seed${seed}.csv" \
