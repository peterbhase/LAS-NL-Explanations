...
do
  ...
  python t5_rl.py \
  --seed=${seed} \
  --model_name="nli_t5_rl_re_seed${seed}" \
  ...
  --max_seq_len=110 \
  --write_prediction \
  --train_output_file="train_rl_re_seed${seed}.tsv" \
  --eval_output_file="eval_rl_re_seed${seed}.tsv" \
  --test_output_file="test_rl_re_seed${seed}.tsv"

  # train task model using generated explanations and predictions
  python t5_rl.py \
  --seed=${seed} \
  --model_name="nli_t5_rl_re_sim_seed${seed}" \
  --task_base_model="t5-small" \
  --train_data_file="train_rl_re_seed${seed}.tsv" \
  --eval_data_file="eval_rl_re_seed${seed}.tsv" \
  --test_data_file="test_rl_re_seed${seed}.tsv" \
